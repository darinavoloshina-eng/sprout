...
const STAGE_TABLE: Partial<Record<CropKey, Record<PlantedBucket, StageEntry>>> = {
  ...
  blueberries: {
    ...
    w8: { headline: 'Established, ongoing care', detail: 'Feed with an acid-forming fertilizer (the kind made for azaleas/camellias) a couple of times each growing season — regular garden fertilizer can push soil pH the wrong way for blueberries. A young bush may only give a light harvest, if any yet; a mature one should be in full production.', severity: 'fyi' },
  },
  raspberries: {
    ...
    w8: { headline: 'Established, ongoing care', detail: 'Prune out old canes after they finish fruiting to make room for new ones. A young planting may only give a light harvest, if any yet; a mature one should be in full production.', severity: 'fyi' },
  },
  blackberries: {
    w0: { headline: 'Establishing', detail: 'Keep soil consistently moist while canes root in.', severity: 'low' },
    w2: { headline: 'Cane growth', detail: 'New cane growth is establishing. Keep watering steady.', severity: 'low' },
  ...
  figs: {
    ...
    w8: { headline: 'Established, ongoing care', detail: 'Expect a modest crop from a young tree; a mature one should be fruiting reliably most seasons.', severity: 'fyi' },
  },
  lemon: {
    ...
    w8: { headline: 'Established, ongoing care', detail: 'Feed monthly with a balanced citrus fertilizer during active growth (or quarterly with a slow-release granular), stopping a few weeks before your first fall frost. A young tree may not fruit much yet; a mature one should be in full production.', severity: 'fyi' },
  },
  lime: {
  ...
  kumquat: {
    ...
    w2: { headline: 'Leafing out', detail: 'New growth should be picking up. Let the top inch of soil dry between waterings.', severity: 'low' },
    w4: { headline: 'Building root reserves', detail: 'Keep watering steady and hold off on heavy feeding until established.', severity: 'low' },
    w8: { headline: 'Established, ongoing care', detail: 'Feed on the same monthly (or quarterly slow-release) citrus schedule as other citrus, stopping a few weeks before your first fall frost. A young tree may not fruit much yet; a mature one should be in full production.', severity: 'fyi' },
  },
  olive: {
    ...
    w8: { headline: 'Established, ongoing care', detail: 'More cold-tolerant than citrus once established, but a young tree or container still benefits from winter shelter. Olive is a light feeder: 2-3 applications through the growing season is plenty, and overfeeding costs you fruit and oil quality, not gains it. Olives are slow to bear, often several years, so a young tree not fruiting yet is normal.', severity: 'fyi' },
  },
  avocado: {
    ...
    w8: { headline: 'Established, ongoing care', detail: 'Avocado is slow to fruit, often taking several years even in ideal conditions, so a young tree not fruiting yet is normal. Once established, feed roughly every 5 weeks through the growing season with a citrus/avocado-formulated fertilizer, watering deeply afterward to flush excess salts.', severity: 'fyi' },
  },
  pomegranate: {
    w0: { headline: 'Establishing', detail: 'Keep soil consistently moist while roots establish.', severity: 'low' },
    w2: { headline: 'Leafing out', detail: 'New growth should be picking up. Keep watering steady.', severity: 'low' },
    w4: { headline: 'Building root reserves', detail: 'Keep watering steady while the root system develops.', severity: 'low' },
    w8: { headline: 'Established, ongoing care', detail: 'More cold-hardy than citrus and fairly drought-tolerant once established. A few feedings through the growing season is enough; too much nitrogen means leaves over fruit. A young tree may take a couple years to fruit well; a mature one should bear reliably each season.', severity: 'fyi' },
  },
  peach: {
    w0: { headline: 'Establishing', detail: 'Keep soil consistently moist while roots establish. Good drainage matters more than extra water here.', severity: 'low' },
    w2: { headline: 'Leafing out', detail: 'New growth should be picking up. Keep watering steady as the root system develops.', severity: 'low' },
    w4: { headline: 'Building root reserves', detail: 'Thin any fruit that sets this first year so the tree puts its energy into roots and structure instead.', severity: 'low' },
    w8: { headline: 'Established, ongoing care', detail: 'Prune to an open center shape each winter while it is dormant. A young tree usually takes two to three years to bear a real harvest; a mature one should fruit reliably each season.', severity: 'fyi' },
  },
  cherry: {
    w0: { headline: 'Establishing', detail: 'Keep soil consistently moist while roots establish. Good drainage matters more than extra water here.', severity: 'low' },
    w2: { headline: 'Leafing out', detail: 'New growth should be picking up. Keep watering steady as the root system develops.', severity: 'low' },
    w4: { headline: 'Building root reserves', detail: 'Keep watering consistent while the tree establishes its structure.', severity: 'low' },
    w8: { headline: 'Established, ongoing care', detail: 'A young tree usually takes several years to bear a real harvest, longer than most other fruit trees. Once it starts fruiting, netting against birds becomes the main challenge.', severity: 'fyi' },
  },
  marigold: {
    w0: { headline: 'Establishing', detail: 'Keep soil consistently moist while roots take hold.', severity: 'low' },
